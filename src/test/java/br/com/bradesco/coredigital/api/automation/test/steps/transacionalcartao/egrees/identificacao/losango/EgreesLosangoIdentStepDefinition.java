package br.com.bradesco.coredigital.api.automation.test.steps.transacionalcartao.egrees.identificacao.losango;
...
public class EgreesLosangoIdentStepDefinition extends AbstractStepDefinition {
    ...
    private final String JSON_EGREES_SCHEMA_NAME = "card_identification_egrees_losango_schema";

    private final String ROOT_JSON_DATA_PATH = "src/main/resources/json_data/card/";

    private final String JSON_EGREES_DATA_NAME = "card_identification_egrees_losango_data";

    private final String URL_CARTAO_IDENTIFICACAO_LOSANGO_EGREES = "/egress-gateway/losango/v1/credit-cards-identification";

    @Given("que seja feito uma requisição GET no endpoint de Identificação de Cartões")
    public void que_seja_feito_uma_requisicao_get_no_endpoint_de_identificacao_de_cartoes() {

        Headers header = headerUtils.getHeader(
                new Header("identification", "178582x8582199"),
                new Header("brand", "LOS"));

        identificacaoEgreesLosangoResponse = fornecimentoService.getResponseListData(ProjectType.CARD_IDENTIFICATION_LOSANGO_EGREES,
                ROOT_JSON_DATA_PATH, JSON_EGREES_DATA_NAME, header,
                IdentificationEgreesLosango.class, URL_CARTAO_IDENTIFICACAO_LOSANGO_EGREES);
